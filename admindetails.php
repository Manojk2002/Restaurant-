...
$_result = mysqli_query($con, "SELECT * FROM `rms_admin` WHERE `id` = '$user'");
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $delete = mysqli_query($con,"DELETE FROM `rms_admin` WHERE `id`= '$id'");
}
?>
...
<head>
  ...
  <title>Display Admin's Data</title>
</head>
...
<body>
  ...
  if (mysqli_num_rows($_result) > 0) { 
      $rowt = mysqli_fetch_array($_result);
      ?>
        <center>
          <div class="col-lg-6 mt-5">
            <div class="card bg-white mb-3 text-center shadow p-3 mb-3 rounded h-30">
            
              <h5>Your Details</h5>
              <br>
              <br>
              <strong>Name : <?php echo " ".$rowt["name"]; ?> </strong>
              <strong>Email :<?php echo " ".$rowt["email"]; ?></strong>
              <strong>Mobile Number : <?php echo " ".$rowt["mobile_no"]; ?></strong>
              <strong>Gender : <?php echo " ".$rowt["gender"]; ?></strong>
              <strong>Address : <?php echo " ".$rowt["address"]; ?></strong>
              <strong>Password : <?php echo " ".$rowt["password"]; ?></strong>
              <div class="row mx-md-n5">
              <a style="margin-left: 150px;"  href="adminupdate.php?id=<?php  echo $rowt["id"]; ?>" class="btn btn-success  mt-5 col-lg-2">Update</a>
              <a style="margin-left: 120px;" href="index.php" class="btn btn-success mt-5 col-lg-2">Back</a> </div>
            </div>
          </div>
        </center>
       <?php
