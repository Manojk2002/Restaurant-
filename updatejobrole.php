...
if (count($_POST) > 0) {
    mysqli_query($con, "UPDATE `rms_jobrole` SET  `name`='" . strtoupper($_POST["role"]) . "',`salary`='" . $_POST["salary"] . "' WHERE `id` ='" . $_POST["id"] . "'");
    header("location:displayjobrole.php");
}
$result = mysqli_query($con, "SELECT * FROM `rms_jobrole` WHERE id ='" . $_GET["id"] . "'");
       ...
       <div style="background-color: azure; height: 650px;" class="d-flex" id="wrapper">
        ...
        <div class="container-fluid">
            <div class="container mt-3">
                <h2>Update Job Role</h2>
                <form action="" method="POST">

                    <div class="mb-3 mt-3">
                        <label for="item">Job Role Name:</label>
                        <input type="text" class="form-control" id="item" value="<?php echo $row['name']; ?>" name="role" required>
                    </div>
                    <input type="hidden" name="id" value="<?php echo $row['id']; ?>">
                    <div class="mb-3 mt-3">
                        <label for="item">Salary:</label>
                        <input type="number" class="form-control" id="item" value="<?php echo $row['salary']; ?>" name="salary" required>
                    </div>

                    <input type="submit" name="submit" value="Save" class="btn btn-success"></input>

                    <a href="displayjobrole.php" class="btn btn-success" role="button">Back</a>
                </form>
            </div>
            <div>
