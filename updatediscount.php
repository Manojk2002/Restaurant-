...
if (count($_POST) > 0) {
    $nmx = strtoupper($_POST["r1"]);
    mysqli_query($con, "UPDATE `rms_discount` SET `percentage`='" . $_POST["r2"] . "',`name`='" .$nmx. "',`purchese amount`='" . $_POST["r3"] . "',`manage by`='" . $_POST["mb"] . "' WHERE `id`='" . $_POST["idd"] . "'");
    header("location:discountdetails.php");
}
$result = mysqli_query($con, "SELECT * FROM `rms_discount` WHERE id ='" . $_GET["id"] . "'");
        ...
        <div class="container-fluid">
            <div class="container mt-3">
                <form action="" method="post">
                    ...
                    <input type="hidden" name="mb" value="<?PHP echo $user; ?>">
                    <div class="row">
                        ...
                        <div class="form-group mt-3">
                            ...
                            <input style="font-size: 12px;" class="input-group input-group-sm p-1" type="number" name="r2" class="form-control" id="" required value="<?PHP echo $row["percentage"]; ?>">
                        </div>
                        <br>
                        <div class="form-group mt-3">
                            <label for="PA" class="">On Or Above Purchase Amount Of:</label>
                            <input style="font-size: 12px;" class="input-group input-group-sm p-1" type="number" name="r3" class="form-control" id="" required value="<?PHP echo $row["purchese amount"]; ?>">
                        </div>
                        <br>
                    </div>
                    <br>
                    <div class="row">
                        <center>
                            <p> <input type="submit" value="Update" name="submit" class="btn btn-success ml-3"> </p>
                        </center>
                    </div>
                </form>
            </div>
